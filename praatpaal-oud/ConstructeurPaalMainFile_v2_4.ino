...
// Timing variables:
/*********************************************/
unsigned long debounceTimeout = 500; // Debounce time (500 ms = 0.5 s)
unsigned long skillTimeout = 300000; // Tijd totdat een skill in werking treed (300000 ms = 300 s = 5 min)
unsigned long toegangTimeout = 10000; // Tijd voordat het scherm weer uit gaat (10000 ms = 10 s)
unsigned long rdyTimeout = 1000; // Tijd voordat er nog een tag gelezen wordt. (1000 ms = 1 s)
unsigned long lichtTimeout = 130; // Tijd tussen stapjes van achtergrond licht. (130 ms)
...
void setup()
{
  ...
  groen1Val = 0;
  blauw1Val = 0;
  setKeystoneLED(rood1Val,groen1Val,blauw1Val);
  rood2Val = 40;
  groen2Val = 1;
  blauw2Val = 150;
  setBaseLED(rood2Val,groen2Val,blauw2Val);
...
void loop(){
  ...
  if((millis() - toegangTimer) >= toegangTimeout){
    tekenHoofdMenu = true;
    toegang = false;
    myGLCD.clrScr();
    skillTimer = millis();
  ...
  if(skillUitvoeren){
    if((millis() - skillTimer) >= skillTimeout){                // Vergelijk of de speler genoeg tijd heeft besteed
      skillTimer = millis();                                    // Reset de timer
...
void lichtAanpassen(){
  if(toegang){
    if((millis() - lichtTimer) >= lichtTimeout){
      lichtTimer = millis();
      if(!skillUitvoeren){
        ...
        setKeystoneLED(100,groen1Val,10);
      }
      else{
        setKeystoneLED(50,groen1Val,blauw1Val);
        if(groen1Val > 254)
          updown = -1;
        if(blauw1Val > 254)
          updown = 1;
...
/*********************************************/
/* Voer de betreffende knop functie uit      */
/*********************************************/
byte voerKnopCommandoUit(byte knopNummer){
  // Als knop Omhoog
  if (knopNummer = 1){
    if(subMenu){
      menuIndex = savedMenuIndex;
    ...
    return 0;
    // Toevoegen: omhoog vanuit Put tekenen
  }
  
  // Als knop Rechts
  else if (knopNummer = 2){
    menuIndex++;
    if(subMenu){
  ...
  // Als knop Onder
    ...
    if(subMenu){
      ...
      // Alles ontkoppelen skill
      else if(menuFiles[menuIndex] == "Menu03.RAW"){
        skillTimer = millis();
        skillUitvoeren = true;
        updateMenu = true;
        blauw1Val = 255;
      ...
      // Item Aansluiten submenu
      else if(menuFiles[menuIndex] == "Menu04.RAW"){
        savedMenuIndex = menuIndex;
        menuIndex = 0;
        subMenu = true;
        updateMenu = true;
  ...
  // Als knop Links
  else if(knopNummer = 4){
    menuIndex--;
    if(subMenu){
      if(menuIndex < 0)
        menuIndex = menuIndexMax;
        while(itemFiles[menuIndex] == "")
          menuIndex--;
    }
    else if(menuIndex < 1){
      menuIndex = menuIndexMax;
      while(menuFiles[menuIndex] == "")
        menuIndex--;
    }
    updateMenu = true;
    return 0;
...
void setKeystoneLED(byte red, byte green, byte blue){
  analogWrite(rood1,red);
  analogWrite(groen1,green);
  analogWrite(blauw1,blue);
}

void setBaseLED(byte red, byte green, byte blue){
  analogWrite(Rood_2,red);
  analogWrite(Groen_2,green);
  analogWrite(Blauw_2,blue);
}
